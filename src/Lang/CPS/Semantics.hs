...
tickM δ μ c = do
  ...
  modifyP (lexicalTimeP μ $ ψ δ μ) $ tick (lexical μ) $ Ψ c -- e s
  modifyP (dynamicTimeP μ $ ψ δ μ) $ tick (dynamic μ) $ Ψ c -- e s
...
-- 'Standard' semantics --
exec :: (Analysis δ μ m) => P δ -> μ -> P m -> Call -> SS m Call
exec δ μ m c = poiter (mstep $ call δ μ m) $ unit c

-- Collecting semantics --
collectExec :: (Analysis δ μ m) => P δ -> μ -> P m -> Call -> SS m Call
...
callTouched :: (Analysis δ μ m) => P δ -> μ -> P m -> Env μ (Ψ δ μ) -> Call -> Set (Addr μ (Ψ δ μ))
callTouched δ μ m e c = closureTouched δ μ m e [] c

closureTouched :: (Analysis δ μ m) => P δ -> μ -> P m -> Env μ (Ψ δ μ) -> [Name] -> Call -> Set (Addr μ (Ψ δ μ))
closureTouched δ μ m e xs c = useMaybeSet . index (runEnv e) *$~ freeVarsLam δ μ m xs c

addrTouched :: (Analysis δ μ m) => P δ -> μ -> P m -> Store δ μ (Ψ δ μ) -> Addr μ (Ψ δ μ) -> Set (Addr μ (Ψ δ μ))
...
collectExecGC :: (Analysis δ μ m) => P δ -> μ -> P m -> Call -> SS m Call
