...
type Analysis δ μ m =
  ...
  , Δ δ μ
  , MonadCPS δ μ m
  , PartialOrder (SS (m δ μ) Call)
  , JoinLattice (SS (m δ μ) Call)
  , JoinLattice (Val δ μ)
  )
...
new :: (Analysis δ μ m) => P δ -> μ -> Name -> m δ μ (Addr μ)
new _ μ x = do
  lτ <- getP $ lexicalTimeP μ ψ
  dτ <- getP $ dynamicTimeP μ ψ
  return $ Addr x lτ dτ

bind :: (Analysis δ μ m) => P δ -> μ -> Name -> Val δ μ -> m δ μ ()
bind δ μ x vD = do
  l <- new δ μ x
  modifyL (envL μ) $ pinsert x l
  modifyL (storeL δ μ) $ pinsertWith (\/) l vD

var :: (Analysis δ μ m) => P δ -> μ -> Name -> m δ μ (Val δ μ)
var δ μ x = do
  e <- getL $ envL μ
  s <- getL $ storeL δ μ
  useMaybeZero $ index s *$ index e $ x

atom :: (Analysis δ μ m) => P δ -> μ -> Atom -> m δ μ (Val δ μ)
...
tickM :: (Analysis δ μ m) => P δ -> μ -> Call -> m δ μ ()
tickM δ μ c = do
  _ <- getP $ envP μ
  _ <- getP $ storeP δ μ
  modifyL (lexicalTimeL μ ψ) $ tick (lexical μ) c
  modifyL (dynamicTimeL μ ψ) $ tick (dynamic μ) c

call :: (Analysis δ μ m) => P δ -> μ -> P m -> Call -> m δ μ Call
call δ μ _ (If a tc fc) = do
  b <- elimBoolM δ *$ atom δ μ a
  return $ if b then tc else fc
call δ μ _ (App fa aes) = do
...
-- 'Standard' semantics --
exec :: (Analysis δ μ m) => P δ -> μ -> P m -> Call -> SS (m δ μ) Call
exec δ μ m = poiter (mstep $ call δ μ m) . munit (mP m δ μ)

-- Collecting semantics --
execCollect :: (Analysis δ μ m) => P δ -> μ -> P m -> Call -> SS (m δ μ) Call
...
execCollectWith :: (Analysis δ μ m) => P δ -> μ -> P m -> Action Call -> (Call -> SS (m δ μ) Call)
